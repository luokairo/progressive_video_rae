...
from progressive_videorae.model.types import ProgressiveState
...
def test_real_wan_decoder_cache_matches_full_sequence_without_weights():
    ...
    typed_state = ProgressiveState(
        tokens=state,
        layout_version=model.contract.layout_version,
        layout_checksum=model.contract.layout_checksum,
        latent_types=torch.tensor([0, 1, 1]),
        contract=model.contract,
    )
    with torch.no_grad():
        full = model.decode(state, cache_mode="disabled").video
        cached = model.decode(
            typed_state, cache_mode="reset", sequence_id="integration"
        ).video
